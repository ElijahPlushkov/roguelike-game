...
import {CHAPTERS} from "./slugs.js";
import {chapterOneSlugs} from "./slugs.js";
...
//load data
function loadLevelData(slug = CHAPTERS.CHAPTER_1) {
...
function loadDialogueData(slug = chapterOneSlugs.DIALOGUES) {
...
function loadEventData(slug = chapterOneSlugs.EVENTS) {
    fetch(`/roguelike-game/load-script?slug=${encodeURIComponent(slug)}`)
        .then(response => response.json())
        .then(events => {
            eventData = events;
...
function loadDoorData(slug = chapterOneSlugs.DOORS) {
    fetch(`/roguelike-game/load-script?slug=${encodeURIComponent(slug)}`)
        .then(response => response.json())
        .then(doors => {
            doorData = doors;
...
function loadItemData(slug = chapterOneSlugs.ITEMS) {
    fetch(`/roguelike-game/load-script?slug=${encodeURIComponent(slug)}`)
        .then(response => response.json())
        .then(items => {
            itemData = items;
...
function loadEnemyData(slug = chapterOneSlugs.ENEMIES) {
    fetch(`/roguelike-game/load-script?slug=${encodeURIComponent(slug)}`)
        .then(response => response.json())
        .then(enemies => {
            enemyData = enemies;
